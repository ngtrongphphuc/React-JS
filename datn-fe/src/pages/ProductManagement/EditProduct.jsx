...
const EditProduct = () => {
  ...
  const onFinish = async ({ image, ...values }) => {
    try {
      ...
      if (image && image.file?.status !== "removed") {
        imageUploaded = await cloudinaryUpload(image.file);
      }
      ...
      message.success("Cập nhật SP thành công");
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <h1 className="font-semibold text-xl">Cập nhật SP</h1>
      ...
      <Form form={form} className="mt-6" layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item name="image" label="Ảnh SP">
